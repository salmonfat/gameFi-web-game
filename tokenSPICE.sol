...
contract tokenSPICE is IERC20,Ownable{
    ...
    address public SPICEandBUSDPair;
    ...
    constructor(address _reward, address _farm){
        ...
        SPICEandBUSDPair = IPancakeFactory(pancakeV2Pouter.factory()).createPair(address(this), BUSDAddress); 
        noSlippage[msg.sender]=true;
        noSlippage[address(this)]=true;
        isSwapingAndAddingLiq=false;
        slippageFee=[5,0,0]; //比例先學黃金礦工   之後做調整
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(
          ...
          balanceOf(SPICEandBUSDPair) > 0 && 
          !isSwapingAndAddingLiq &&
          from != address(pancakeV2Pouter) && 
          (from == SPICEandBUSDPair || to == SPICEandBUSDPair)
        ) {
        ...
        bool takeFee = false;
        if (balanceOf(SPICEandBUSDPair) > 0 && (from == SPICEandBUSDPair || to == SPICEandBUSDPair)) {
          takeFee = true;
        }
